# draft framework gallery

## k-ops
| day | taks | output |
|---|---|---|
| monday-tuesday | slide | template |
| wednesday-thursday | graffiti | assets |
| thursday-friday | photo | assets |

## staff
| day | taks | output |
|---|---|---|
| monday-tuesday | painting | assets |
| tuesday-thursday | live report | template | 


## Drafting for creating gallery

## 1 week

## worker : k-ops

| day | task | output | time |
|---|---|---|---|
| **slide** |---|---|---|
| monday | gethering research data | yyyy-mm-dd_reserch_data.md | 60 minute |
|  | analyze research | yyyy-mm-dd_analyze_reserch.md | 10 minute |
|  | guideline | yyyy-mm-dd_[theme name]_guideline.md | 60 minute |
| tuesday | creating design | link canva | 20 minute |
|  | review | status project | 20 minute |
|  | promotion planning | yyyy-mm-dd_promotion_planning.md | 60 minute |
|  | create design promotion | link canva | 10 minute |
|  | upload design promotion | link post | 10 minute |
| **graffiti** |---|---|---|
| wednesday | download graffiti | link drive | 15 menit |
|  | reserch metadata | yyyy-mm-dd-metadata.md | 50 menit |
|  | review | status project | 20 menit |
|  | upload to website | link website | 10 menit |
|  | promotion planning | yyyy-mm-dd_promotion_planning.md | 15 menit |
| thursday | promotion planning | yyyy-mm-dd_promotion_planning.md | 45 menit |
|  | create design promotion | link post | 10 menit |
|  | upload | link post | 10 menit |
| **photo** |---|---|---|
| thursday | download photo | link drive | 15 menit |
|  | reserch metadata | yyyy-mm-dd-metadata.md | 50 menit |
|  | review | status project | 20 menit |
| friday | upload to website | link website | 10 menit |
|  | promotion planning | yyyy-mm-dd_promotion_planning.md | 60 menit |
|  | create design promotion | link post | 10 menit |
|  | upload | link post | 10 menit |

## worker : staff

| day | task | output | time |
|---|---|---|---|
| **live reaport** |---|---|---|
| tuesday | gethering research data | yyyy-mm-dd_research_data.md | 60 minute |
| wednesday | analyze research | yyyy-mm-dd_research_analyze.md | 10 minute |
| wednesday | guideline | yyyy-mm-dd_[theme name]_guideline.md | 60 minute |
| wednesday | creating design | link canva | 60 minute |
| thursday | review | status project | 20 minute |
| thursday | promotion planning | yyyy-mm-dd_promotion_planning.md | 30 minute |
| thursday | promotion planning | yyyy-mm-dd_promotion_planning.md | 30 minute |
| thursday | create design promotion | link canva | 10 minute |
| thursday | upload design promotion | link post | 10 minute |
| **painting** |---|---|---|
| monday | download painting | link drive | 15 menit |
|  | reserch metadata | yyyy-mm-dd-metadata.md | 50 menit |
|  | review | status project | 20 menit |
|  | upload to website | link website | 10 menit |
|  | promotion planning | yyyy-mm-dd_promotion_planning.md | 25 menit |
| tuesday | promotion planning | yyyy-mm-dd_promotion_planning.md | 35 menit |
|  | create design promotion | link post | 10 menit |
|  | upload | link post | 10 menit |



### monev 

| day | task | output | time |
|---|---|---|---|
| 1 week after | report view | yyyy-mm-dd_report_view | 10 minute |
